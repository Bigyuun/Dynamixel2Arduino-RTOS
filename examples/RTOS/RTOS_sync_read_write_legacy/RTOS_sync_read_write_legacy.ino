...
int32_t homing_offset[DXL_ID_CNT];
uint8_t operation_mode[DXL_ID_CNT];
uint8_t torque_enable[DXL_ID_CNT];
uint8_t moving[DXL_ID_CNT] ;
int16_t present_cur[DXL_ID_CNT];
int32_t present_vel[DXL_ID_CNT];
int32_t present_pos[DXL_ID_CNT];
    ...
    if (state_write_signal) {
      if (monitoring_flag) {
        for(int i=0; i<DXL_ID_CNT; i++){
          ...
          SERIAL.print(", Homing Offset: ");
          SERIAL.print(homing_offset[i]);
...
void dxl_state_read(){
  ...
  if(recv_cnt_cvp > 0) {
    for(uint8_t i = 0; i < recv_cnt_cvp; i++) {
      ...
      present_pos[i] = sr_data[i].present_position - homing_offset[i];
    }
  } else {
      ...
      */
      ...
      success = [v_command](uint8_t len) -> bool
      {
        ...
        if (target_name == "current") {
          ...
          SERIAL.println(target_name);
          for (int i=0; i<DXL_ID_CNT; i++) {
            sw_data[i].goal_position = (int32_t)(v_command[2+i].toInt()) + homing_offset[i];
...
bool homing(){
  ...
  */
  ...
  while(true) {
    ...
    for (int i=0; i<DXL_ID_CNT; i++) {
      ...
      else if (limit_sensor_signal[i] == LOW && sw_data[i].goal_velocity != 0) {
  ...
  * @author DY
  * @note
  *   There exist the error term when you don't set homming offset zero.
  *   So, we set Dynamixel Homing Offset 0, and use virtual offset values.
  *   --> command 'setGoalPosition' = target + home offset;
  */
  dxl.torqueOff(BROADCAST_ID); SERIAL.println("Torque Off...");
  for (int i=0; i<DXL_ID_CNT; i++) {
  ...
  for (int i=0; i<DXL_ID_CNT; i++) {
    ...
    // sw_data[i].goal_position = homing_offset[i];
  }
  sw_infos_goal_position.is_info_changed = true;
  dxl.syncWrite(&sw_infos_goal_position);
