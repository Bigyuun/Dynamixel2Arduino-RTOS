...
static void thread_serial_write( void *pvParameters ) 
{
  ...
  uint8_t hardware_err[DXL_ID_CNT];
  ...
  while(1){
    ...
    if(recv_cnt_he > 0) {
      for(uint8_t i = 0; i < recv_cnt_he; i++) {
        hardware_err[i] = sr_data[i].hardware_error;
      }
    } else {
      SERIAL.print("SyncRead Current & Velocity & Position Fail, Lib error code: ");
      SERIAL.println(dxl.getLastLibErrCode());
    ...
    if (monitoring_flag) {
      ...
      for(int i=0; i<DXL_ID_CNT; i++){
        char msg[50];
        sprintf(msg, "/%d,%d,%d,%f,%f,%f,%d;", i+1, torque_enable[i], moving[i], present_cur[i], present_vel[i], present_pos[i], hardware_err[i]);
        // xSemaphoreTake(serial_mutex, portMAX_DELAY);
        SERIAL.println(msg);
        SERIAL.flush();
        // xSemaphoreGive(serial_mutex);
...
void dynamixel_init()
{
  ...
  // present_cur & vel & pos =============================================
  sr_infos_present_cvp.packet.p_buf = user_pkt_buf_present_position;
